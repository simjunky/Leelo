...
function read_model_data()::ModelData
    ...
    model_data = ModelData(interest_rate =  my_interest_rate,
                        ...
                        minCapacityPotCT = Matrix{Float64}(converter_data[!, Symbol.("minCapacityPotCTb" .* string.(collect(1:n_buses)))]),
                        maxCapacityPotCT = Matrix{Float64}(converter_data[!, Symbol.("maxCapacityPotCTb" .* string.(collect(1:n_buses)))]),
                        ...
                        minVolumePotST = Matrix{Float64}(storage_data[!, Symbol.("minVolumePotSTb" .* string.(collect(1:n_buses)))]),
                        maxVolumePotST = Matrix{Float64}(storage_data[!, Symbol.("maxVolumePotSTb" .* string.(collect(1:n_buses)))]),
                        ...
                        costFictitiousFlows = Float64(scenario_setting_data[1, :CostFicticiousFlow]),
