...
function add_model_variables(model::JuMP.Model, config::AbstrConfiguration, data::ModelData)
    ...
    @variable(model, powerG[1:n_timesteps, 1:n_buses, 1:n_conv_generators] >= 0)

    # (t,h) power delivered by hydro power generator h during timestep t
    @variable(model, powerH[1:n_timesteps, 1:n_hydro_generators] >= 0)

    # WARNING: changed from powerPpump
    # (t,h) power consumed by hydro power plant h for pumping during timestep t
    @variable(model, powerHpump[1:n_timesteps, 1:n_hydro_generators] >= 0)
    ...
    @variable(model, powerR[1:n_timesteps, 1:n_buses, 1:n_ren_generators] >= 0)

    # (t,b) power unserved in bus b during timestep t
    @variable(model, powerunserved[1:n_timesteps, 1:n_buses] >= 0)
    ...
    @variable(model, powerLpos[1:n_timesteps, 1:n_lines] >= 0)

    # (t,l) power flow on line l (negative ie from b to a)
    @variable(model, powerLneg[1:n_timesteps, 1:n_lines] >= 0)

    # (t,l) power losses on line l
    @variable(model, powerLlosses[1:n_timesteps, 1:n_lines] >= 0)
    ...
    @variable(model, reserveOperatingCT[1:n_timesteps, 1:n_buses, 1:n_conversion_technologies] >= 0)

    # (t,h) offered operating reserve by hydroreservoir h during timestep t
    @variable(model, reserveOperatingH[1:n_timesteps, 1:n_hydro_generators] >= 0)

    # (t,b,g) offered operating reserve by conventional generator g in bus b during timestep t
    @variable(model, reserveOperatingG[1:n_timesteps, 1:n_buses, 1:n_conv_generators] >= 0)
    ...
    @variable(model, storedST[1:n_timesteps, 1:n_buses, 1:n_storage_technologies] >= 0)

    # TODO: is this really needed?
    # (b,st) initial energy storage of storage technology st in bus b during t=1
    @variable(model, vIniST[1:n_buses, 1:n_storage_technologies] >= 0)

    # TODO: is this really needed?
    # (b,st) final energy storage of storage technology st in bus b during t=end
    @variable(model, vFinST[1:n_buses, 1:n_storage_technologies] >= 0)

    # (t,b,st) energy losses of storage technology st in bus b during timestep t
    @variable(model, vLossST[1:n_timesteps, 1:n_buses, 1:n_storage_technologies] >= 0)
    ...
    set_lower_bound.(pG, repeat(transpose(max.(data.pMinG / 1000, 0.0)), n_buses, 1))
    # upper boundary: upper bound of a generator g given by pMaxG[g] for every bus b adjusted from MW to GW
    set_upper_bound.(pG, repeat(transpose(data.pMaxG / 1000), n_buses, 1))

    # (l) built capacity of transmission line l in [GW]
    @variable(model, pL[1:n_lines] >= 0)
    ...
    @variable(model, pH[1:n_hydro_generators] >= 0)
    # upper bound given by not yet realized capacity expansion (max - existing) adjusted from MW to GW
